...
from retnets.torch.component.multiscale_retention import MultiScaleRetention
...
class RetNetRelPos(nn.Module):
    def __init__(self, args):
        ...
        angle = 1.0 / (10000 ** torch.linspace(0, 1,
                       args.decoder_embed_dim // args.decoder_retention_heads // 2))
        angle = angle.unsqueeze(-1).repeat(1, 2).flatten()
        decay = torch.log(
            1 - 2 ** (-5 - torch.arange(args.decoder_retention_heads, dtype=torch.float)))
    ...
    def forward(self, slen, activate_recurrent=False, chunkwise_recurrent=False):
        if activate_recurrent:
            ...
            mask = torch.tril(torch.ones(
                self.recurrent_chunk_size, self.recurrent_chunk_size).to(self.decay))
...
class DecoderLayer(nn.Module):
    def __init__(
        ...
        self.retention_layer_norm = LayerNorm(
        ...
        **kwargs
        ...
        if args.layernorm_embedding:
            self.layernorm_embedding = LayerNorm(
                embed_dim, eps=args.layernorm_eps)
        else:
    ...
    def forward(
        ...
        **kwargs
        ...
        if self.chunkwise_recurrent and prev_output_tokens.size(1) % self.recurrent_chunk_size != 0:
            padding_len = self.recurrent_chunk_size - \
        ...
        retention_rel_pos = self.retnet_rel_pos(
            slen, incremental_state is not None and not is_first_step, chunkwise_recurrent=self.chunkwise_recurrent)

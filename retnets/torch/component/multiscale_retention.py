...
class MultiScaleRetention(nn.Module):
    def __init__(
        ...
        self.k_proj = MultiwayWrapper(
            args, nn.Linear(embed_dim, embed_dim, bias=True))
        self.v_proj = MultiwayWrapper(args, nn.Linear(
            embed_dim, embed_dim * self.factor, bias=True))
        self.g_proj = MultiwayWrapper(args, nn.Linear(
            embed_dim, embed_dim * self.factor, bias=True))

        self.out_proj = MultiwayWrapper(args, nn.Linear(
            embed_dim * self.factor, embed_dim, bias=True))

        self.group_norm = MultiwayWrapper(args, LayerNorm(
            self.head_dim, eps=args.layernorm_eps, elementwise_affine=False))
    ...
    def parallel_forward(self, qr, kr, v, mask):
        ...
        vr = v.view(bsz, tgt_len, self.num_heads,
    ...
    def recurrent_forward(
        ...
        if "prev_key_value" in incremental_state:
            ...
            kv = prev_kv * (1 - 1 / scale).view(self.num_heads,
                                                1, 1) + kv / scale.view(self.num_heads, 1, 1)
    ...
    def chunk_recurrent_forward(
        ...
        qr = qr.view(bsz, self.num_heads, num_chunks,
                     chunk_len, self.key_dim).transpose(1, 2)
        kr = kr.view(bsz, self.num_heads, num_chunks,
                     chunk_len, self.key_dim).transpose(1, 2)
        v = v.view(bsz, num_chunks, chunk_len, self.num_heads,
        ...
        kv = kv.view(bsz, num_chunks, self.num_heads,
                     self.key_dim, self.head_dim)

        kv_recurrent = []
        cross_scale = []
        kv_state = torch.zeros(bsz, self.num_heads,
                               self.key_dim, self.head_dim).to(v)
    ...
    def forward(
        ...
        if incremental_state is not None:
            output = self.recurrent_forward(
                qr, kr, v, inner_mask, incremental_state)
